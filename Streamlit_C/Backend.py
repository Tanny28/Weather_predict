...
def get_prediction(city, period):
    if city in cities:
        days = {"1 month": 30, "6 months": 180, "1 year": 365, "2 years": 730}.get(period)
        ...
        if days:
            ...
            prediction_results = {
                "LSTM": {
                    "temperature": float(lstm_predictions[-1, 0]),
                    "humidity": float(lstm_predictions[-1, 1]),
                    "rainfall": float(lstm_predictions[-1, 2]),
                    "wind_speed": float(lstm_predictions[-1, 3])
                },
                "Linear Regression": {
                    "temperature": float(lr_predictions['temperature'][-1]),
                    "humidity": float(lr_predictions['humidity'][-1]),
                    "rainfall": float(lr_predictions['rainfall'][-1]),
                    "wind_speed": float(lr_predictions['wind_speed'][-1])
    }
            }
        ...
        raise ValueError("City not in the list of supported cities")
